...
var dx = (xOffset.max - xOffset.min) / points;
var dy = (yOffset.max - yOffset.min) / lines;
var mx = (xOffset.min + xOffset.max) / 2;
...
var randNormal = function randNormal(mu, sigma) {
	var sum = 0;
	for (var i = 0; i < 6; i += 1) {
		sum += rand(-1, 1);
	}
		...
		for (var k = 0; k < points; k++) {
			...
			switch (obj.type) {
				...
				case 'noisy rounded':
					...
					break;
				case 'noisy':
					newY = 0.3 * prevY + 0.7 * (y - 600 * noise + noise * Math.random() * 200 + Math.random());
					break;
			}
			ctx.lineTo(x, newY);
			prevY = newY;
